    ...
    case SIF:
         ...
         if(!ast_check_priv_if(tree->u.selection.cond)){
          ...
          // for priv cond
               ...
               if(has_deref_write > 0){
                  fprintf(output, "transactional_resolve(_picco_map_%d, %d, __s);\n", if_branchnode_height(current) + 1);
                  //if(if_branchnode_height(current) == 0)
                  if(current_branch == B_NONE)
                     fprintf(output, "NULL, %d, __s);\n", if_branchnode_height(current) + 1);
                  else if (current_branch == B_THEN){
                     fprintf(output, "_picco_map_%d, %d, __s);\n", if_branchnode_height(current), if_branchnode_height(current) + 1);
                  }
                  else if (current_branch == B_ELSE){
                     fprintf(output, "_picco_map_%d, %d, __s);\n", if_branchnode_height(current), if_branchnode_height(current) + 1);
                  }
                  else{
                     fprintf(output, "NULL, NULL, NULL, __s);\n // ERROR!!!\n");
                     printf("branch id error");
                  }
               }
               else
               ...
               clear_var_list(var_list, current);
               fprintf(output, "}\n");
         }
      break;
    default:
      ...
      /* *********************************************/
         ...
         if(private_if_index != -1){
            ...
            if(tree->left->ftype == 0){ // int
               fprintf(output, "update_int(");
            }
            else{ //float
               fprintf(output, "update_float(");
            }
            fprintf(output, "_picco_map_%d, %s, ", private_if_index, str_string(leftop));
            for (int i = 0; i < dereferences; i ++)
               fprintf(output, "*");
            fprintf(output, "%s, %d, ", str_string(leftop), private_if_index);
            if(current_branch == B_THEN){
               fprintf(output, "B_THEN);\n");
            }
            else if(current_branch == B_ELSE){
               fprintf(output, "B_ELSE);\n");
            }
            else
               fprintf(output, "B_ERR);\n // ERROR - should be inside a branch, but incorrect branch_id %d\n", current_branch);
         }
            ...
            switch(tree->u.expr->type){
               case ASS: /* Handle assignment statements x = e
                  ...
                  * store all needed data */
                  ...
                  var_list = var_push(var_list, tree->u.expr->left->u.sym->name, tree->u.expr->left->size, tree->u.expr->left->ftype, tree->u.expr->left->isptr, tree->u.expr->left->thread_id, tree->u.expr->left->sizeexp);
                  switch(tree->u.expr->right->type){
                     case PREOP: /* Handle pre-inc/pre-dec ++x/--x */
                        //printf("%s, %d, %d, %d, %d, %d \n", tree->u.expr->right->left->u.sym->name, tree->u.expr->right->left->size, tree->u.expr->right->left->ftype, tree->u.expr->right->left->isptr, tree->u.expr->right->left->thread_id, tree->u.expr->right->left->sizeexp);
                        var_list = var_push(var_list, tree->u.expr->right->left->u.sym->name, tree->u.expr->right->size, tree->u.expr->right->ftype, tree->u.expr->right->isptr, tree->u.expr->right->thread_id, tree->u.expr->right->sizeexp);
                  ...
                  break;
               case PREOP: /* Handle pre-inc/pre-dec ++x/--x */
                  //printf("%s, %d, %d, %d, %d, %d \n", tree->u.expr->left->u.sym->name, tree->u.expr->left->size, tree->u.expr->left->ftype, tree->u.expr->left->isptr, tree->u.expr->left->thread_id, tree->u.expr->left->sizeexp);
                  //fprintf(output, "\n // found PREOP in priv branch! \n");
                  var_list = var_push(var_list, tree->u.expr->left->u.sym->name, tree->u.expr->left->size, tree->u.expr->left->ftype, tree->u.expr->left->isptr, tree->u.expr->left->thread_id, tree->u.expr->left->sizeexp);
                  break;
               case POSTOP: /* Handle post-inc/post-dec x++/x-- */
...
var_node var_push(var_node var_list, char* name, int size, int flt, int ptr, int thread_id, int sizeexp){
   //printf("PUSHING VAR\n");
...
void transactional_initialize(var_node var_list, branchnode current, int max_nest_level){
   if(debug_msg) fprintf(output, "/************ BEGIN TRANSACTIONAL INIT *************/\n");
   /* We are outside of the nested part, so need to increment the nesting level */
   int nest_level = if_branchnode_height(current) + 1;
   /* Set the private conditional */
   set_conditional(nest_level, var_list->thread_id);
   ...
   fprintf(output, "delta _picco_map_%d = NULL;\n", nest_level);
   ...
   while(next_var->name != NULL){
      ...
      if(next_var->isfloat == 1){
         if(next_var->isptr == 1){
            fprintf(output, "update_float_pointer(_picco_map_%d, &%s, %s, %d, INIT);\n", nest_level, next_var->name, next_var->name, nest_level);
         }
         fprintf(output, "update_float(_picco_map_%d, &%s, %s, %d, INIT);\n", nest_level, next_var->name, next_var->name, nest_level);
      }
      else if(next_var->isptr == 1){
         fprintf(output, "update_int_pointer(_picco_map_%d, &%s, %s, %d, INIT);\n", nest_level, next_var->name, next_var->name, nest_level);
      }
      else{
         fprintf(output, "update_int(_picco_map_%d, &%s, %s, %d, INIT);\n", nest_level, next_var->name, next_var->name, nest_level);
      }
      next_var = next_var->next;
   }
...
void clear_var_list(var_node var_list, branchnode current){
   /* We are outside of the nested part, so need to increment the nesting level */
   int nest_level = if_branchnode_height(current) + 1;
